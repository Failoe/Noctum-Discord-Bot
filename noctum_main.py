...
NewMaxStructuresInRange=4000.0
...
TributeDinoExpirationSeconds=86400```"""
    ...
    b = """```TheMaxStructuresInRange=10500.0
...
OverrideMaxExperiencePointsPlayer=1798540
PlayerHarvestingDamageMultiplier=1.0
OverrideMaxExperiencePointsDino=900000
...
StructureDamageRepairCooldown=180```"""
    c = """```PvPZoneStructureDamageMultiplier=6.0
...
OverrideNamedEngramEntries=(EngramClassName="EngramEntry_CompoundArrow_C",EngramLevelRequirement=70,EngramPointsCost=35,EngramHidden=False,RemoveEngramPreReq=True)```"""
    
    await client.send_message(message.message.author, a)
    await client.send_message(message.message.author, b)
    await client.send_message(message.message.author, c)
...
# @online.command(pass_context=True, name="island")
# async def _client(ctx):
#     """The Island Player List"""
#     await client.send_message(ctx.message.author, steamplayers("73.83.15.50"))

# @online.command(pass_context=True, name="scorched")
# async def _client(ctx):
#     """Scorched Earth Player List"""
#     await client.send_message(ctx.message.author, steamplayers("73.83.15.50",27017))

# @online.command(pass_context=True, name="event")
# async def _client(ctx):
#     """Scorched Earth Player List"""
#     await client.send_message(ctx.message.author, steamplayers("24.113.165.206",27015))
